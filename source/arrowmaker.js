// This ArrowMarker is a VML/SVG overlay that shows a coloured arrow on the map
...
function ArrowMarker(point, rotation, color, opacity, tooltip) {

  this.point_ = point;
  this.rotation_ = rotation;
  var r = rotation + 90;//compass to math
  this.dx_ = 10*Math.cos(r*Math.PI/180);//other end of arrow line to point
  this.dy_ = 10*Math.sin(r*Math.PI/180);
  ...
  this.svgId_ = arrowMarkerCounter;
...
ArrowMarker.prototype.initialize = function(map) {
  ...
  if(navigator.userAgent.indexOf("MSIE") != -1){
    ...
    // make a 20x20 pixel space centered on the arrow 
	...
	svgNode.setAttribute("x1",10);
	svgNode.setAttribute("y1",10);
	svgNode.setAttribute("x2",10+this.dx_);
	svgNode.setAttribute("y2",10+this.dy_);
	...
	if(this.rotation_){
      	...
      	svgM.id="arrowMarkerHead"+this.svgId_;
		...
		svgNode.setAttribute("marker-start","url(#arrowMarkerHead" + this.svgId_ + ")");
    }
...
ArrowMarker.prototype.redraw = function(force) {
  ...
  if(navigator.userAgent.indexOf("MSIE") != -1){
	...
	this.svgRoot_.setAttribute("style", "position:absolute; top:"+ (p.y-10) + "px; left:" + (p.x-10) + "px");
  }
