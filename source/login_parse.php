...
//echo "<pre>";
	if(isset($_POST['Submit']))
	{
		...
		if(($_POST['username']) || ($_POST['password']))
		{
			...
			$response=file_get_contents('http://'.$_SERVER["HTTP_HOST"].'/app_proto/web_services/login_api.php?username='.$username.'&password='.$password);
...
//echo $_SERVER["HTTP_HOST"];
	...
	//exit();
		...
		$imei=$array['Result']['Data'][0]['imei'];//echo "<pre>";
		//print_r($array);exit();
		//var_dump($uid);
		//exit();
			header("location:Gmap_location.php?imei=$imei");
		}	
		else
		{
